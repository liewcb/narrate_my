...
internal class AndroidARView(
    ...
    fun onResume() {
        android.util.Log.e("AR_DEBUG", "AndroidARView.onResume called, isARInitialized=$isARInitialized")
        if (!isARInitialized) return
        renderer.needsTextureRecreation = true
        glSurfaceView.onResume()
        resumeSessionInternal()
    }

    fun onPause() {
        android.util.Log.e("AR_DEBUG", "AndroidARView.onPause called, isSessionResumed=$isSessionResumed")
        isSessionResumed = false
        renderer.needsTextureRecreation = true
        // 1. Pause GL rendering FIRST so onDrawFrame stops calling session.update()
        glSurfaceView.onPause()

        // 2. Pause ARCore session AFTER the GL rendering thread has stopped
        try {
            session?.pause()
        } catch (e: Exception) {
            Log.e(TAG, "Error in session.pause(): ${e.message}")
        }

        // 3. Clean up guide and tracking state
    ...
    private fun resumeSessionInternal() {
        ...
        android.util.Log.e("AR_DEBUG", "resumeSessionInternal called: session=${currentSession != null}, isSessionResumed=$isSessionResumed")
        ...
        try {
            ...
            android.util.Log.e("AR_DEBUG", "ARCore session resumed and camera texture re-bound (id=${backgroundRenderer.textureId})")
        } catch (e: Exception) {
            android.util.Log.e("AR_DEBUG", "Error resuming session: ${e.message}, scheduling retry in 250ms...")
            isSessionResumed = false
    ...
    private fun initializeARView(call: MethodCall, result: MethodChannel.Result) {
        ...
        // Now that configuration is complete, start or resume the AR session
    ...
    private inner class ArCoreRenderer : GLSurfaceView.Renderer {
        ...
        @Volatile
        var needsTextureRecreation = false
        ...
        override fun onDrawFrame(gl: GL10?) {
            ...
            try {
                if (!isSessionResumed) {
                    if (session != null && !pendingSessionResume) {
                        pendingSessionResume = true
                        glSurfaceView.postDelayed({
                            pendingSessionResume = false
                            if (session != null && !isSessionResumed) {
                ...
                if (needsTextureRecreation) {
                    val newTexId = backgroundRenderer.reinitializeTexture()
                    session?.setCameraTextureName(newTexId)
                    needsTextureRecreation = false
                    android.util.Log.e("AR_DEBUG", "Forced new camera texture ID: $newTexId and rebound to session on GLThread")
                } else if (backgroundRenderer.textureId != -1) {
                    session?.setCameraTextureName(backgroundRenderer.textureId)
                }
                val frame = session?.update()
                if (frame == null) {
                    android.util.Log.e("AR_DEBUG", "onDrawFrame: session.update() returned NULL, isResumed=$isSessionResumed")
                    return
                }
                android.util.Log.e("AR_DEBUG", "onDrawFrame: frame OK, time=${frame.timestamp}, tracking=${frame.camera.trackingState}")
                currentFrame = frame
                ...
                backgroundRenderer.draw(frame, session)
