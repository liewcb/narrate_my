...
class RegisterVm extends ChangeNotifier {
  ...
  Future<Profile?> registerWithGoogle() async {
    ...
    try {
      ...
      _finishWithError(e, field: 'google');
      return null;
    } catch (_) {
      _finishWithError(
        GoogleSignInFailure(RegisterMessages.m3GoogleSignInFailed),
        field: 'google',
      );
      return null;
    }
  ...
  Future<bool> sendUsernameOtp({
    ...
    try {
      ...
      // Force field: 'username' here rather than falling back to whatever
      // sub-field the repository tagged (e.g. it tags the username tab's
      // OWN phone input 'phone' too, same as the separate Phone tab's
      // sendPhoneOtp does) — the screen only uses fieldError to pick which
      // TAB shows the error, not to highlight one input inside it, so a
      // shared 'phone' tag from two different tabs would be ambiguous.
