            ...
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _Header(vm: vm),
                  const SizedBox(height: 24),
                  _TravelType(selected: vm.travelType, onSelected: vm.setTravelType),
                  const SizedBox(height: 24),
                  _TripName(initialValue: vm.tripName, onChanged: vm.setTripName),
                  const SizedBox(height: 24),
                  _TravelDates(vm: vm),
                  const SizedBox(height: 24),
                  _ExplorationTime(selected: vm.exploration, onSelected: vm.setExploration),
                  const SizedBox(height: 24),
                  _TravelPace(selected: vm.pace, onSelected: vm.setPace),
                  const SizedBox(height: 24),
                  _Interests(
                  ...
                  const SizedBox(height: 24),
                  _Transportation(selected: vm.transportation, onSelected: vm.setTransportation),
              ...
              onPressed: () {
                ...
                if (errors.isNotEmpty) {
                  ScaffoldMessenger.of(context)
                    ..hideCurrentSnackBar()
                    ..showSnackBar(
                      SnackBar(content: Text(errors.values.first), backgroundColor: Colors.red),
                    );
                  return;
                }
                try {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => MustVisitSelectionScreen(draft: draft),
                    ),
                  );
                } catch (e) {
                  ScaffoldMessenger.of(context)
                    ..hideCurrentSnackBar()
                    ..showSnackBar(
                      SnackBar(content: Text(e.toString().replaceFirst('Bad state: ', '')), backgroundColor: Colors.red),
                    );
                }
              },
            ),
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          style: GoogleFonts.playfairDisplay(fontSize: 32, fontWeight: FontWeight.w700, height: 1.15, color: AppColors.brandCharcoal),
        ),
      ],
    );
  }
...
class _TravelType extends StatelessWidget {
  ...
  const _TravelType({required this.selected, required this.onSelected});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Travel Type', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
        const SizedBox(height: 8),
        Row(
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: const [BoxShadow(color: Color(0x0A004D40), offset: Offset(0, 2), blurRadius: 10)],
                    ),
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(icon, size: 22, color: isSelected ? AppColors.brandGreen : AppColors.outline),
...
class _TripName extends StatefulWidget {
  ...
  const _TripName({required this.initialValue, required this.onChanged});
...
class _TripNameState extends State<_TripName> {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Name your trip', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
        const SizedBox(height: 8),
        Container(
          decoration: BoxDecoration(
            ...
            boxShadow: const [BoxShadow(color: Color(0x0A004D40), offset: Offset(0, 4), blurRadius: 16)],
          ),
          child: TextField(
...
class _TravelDates extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    String format(DateTime? d) => d == null ? 'Pick date' : DateFormat('MMM d, yyyy').format(d);
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text('Travel Dates', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
            if (vm.startDate != null)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: AppColors.brandGreenLight, borderRadius: BorderRadius.circular(12)),
                child: Text(
                  '$days day${days == 1 ? '' : 's'}',
        ...
        if (warning != null) ...[
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: coverage == WeatherCoverage.outOfRange ? Colors.red.shade50 : Colors.orange.shade50,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: coverage == WeatherCoverage.outOfRange ? Colors.red.shade200 : Colors.orange.shade200,
              ),
            ),
            child: Row(
              children: [
                Icon(
                  coverage == WeatherCoverage.outOfRange ? Icons.warning_amber_rounded : Icons.info_outline_rounded,
                  size: 18,
                  color: coverage == WeatherCoverage.outOfRange ? Colors.red.shade700 : Colors.orange.shade700,
                ),
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(label, style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1.0, color: AppColors.outline)),
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    date,
                    style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal),
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
              ],
...
class _DateRangePickerDialogState extends State<_DateRangePickerDialog> {
  ...
  bool _isDisabled(DateTime day) {
    if (day.isBefore(widget.firstDate) || day.isAfter(widget.lastDate)) return true;
    final start = _start;
    if (start == null || _hasCompleteRange) return false;
    if (day.isBefore(start)) return true;
    return day.isAfter(widget.maxEndFor(start));
  }

  void _onDayTapped(DateTime day) {
    if (_isDisabled(day)) return;
    setState(() {
      if (_start == null || _hasCompleteRange || day.isBefore(_start!)) {
        _start = day;
        _end = null;
      } else {
        _end = day;
  ...
  Widget build(BuildContext context) {
    ...
    if (_start == null) {
      ...
      hint = 'End date must be between ${DateFormat('MMM d').format(minEnd)} and ${DateFormat('MMM d').format(maxEnd)}';
    } else {
      ...
      child: ConstrainedBox(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Pick travel dates', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.brandCharcoal)),
              const SizedBox(height: 4),
              Text(hint, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.brandGreen)),
              const SizedBox(height: 12),
              Row(
                children: [
                  IconButton(onPressed: canGoPrev ? () => _shiftMonth(-1) : null, icon: const Icon(Icons.chevron_left_rounded), color: AppColors.brandGreen),
                  Expanded(child: Text(DateFormat('MMMM yyyy').format(firstOfShown), textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.brandCharcoal))),
                  IconButton(onPressed: canGoNext ? () => _shiftMonth(1) : null, icon: const Icon(Icons.chevron_right_rounded), color: AppColors.brandGreen),
                ],
              ),
              Row(
                children: _weekdayLabels
                    .map((label) => Expanded(
                  child: Text(label, textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.outline)),
                ))
                    .toList(),
              ),
              const SizedBox(height: 4),
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text('Select', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
                    ),
                  ),
                ],
              ),
  ...
  Widget _buildDayCell(DateTime day) {
    ...
    final inValidWindow = _start != null && _end == null && !day.isBefore(_start!) && !day.isAfter(widget.maxEndFor(_start!));
    ...
    final fg = isSelected ? Colors.white : disabled ? AppColors.outlineLight : AppColors.brandCharcoal;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: ShapeDecoration(
          color: bg,
          shape: CircleBorder(
            side: isToday && !isSelected ? BorderSide(color: AppColors.brandGreen.withOpacity(0.6)) : BorderSide.none,
          ),
        ),
        child: Text(
          '${day.day}',
          style: GoogleFonts.inter(fontSize: 13, fontWeight: isSelected ? FontWeight.w700 : FontWeight.w500, color: fg),
        ),
      ),
    );
  }
...
class _WeatherCoverageBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color, icon) = switch (coverage) {
      WeatherCoverage.full => ('Weather data available for all days', AppColors.brandGreen, Icons.check_circle_rounded),
      WeatherCoverage.primaryOnly => ('Primary forecast only (${ItineraryValidationService.primaryForecastDays}-day)', Colors.orange, Icons.info_outline_rounded),
      WeatherCoverage.outOfRange => ('Beyond weather forecast range', Colors.red, Icons.warning_amber_rounded),
...
class _ExplorationTime extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const options = ['Standard (9 AM - 8 PM)', 'Relaxed (10 AM - 6 PM)', 'Intense (8 AM - 10 PM)'];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Exploration Time', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
        const SizedBox(height: 8),
        Column(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(option, style: GoogleFonts.inter(fontSize: 14, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500, color: isSelected ? AppColors.brandGreen : AppColors.brandCharcoal)),
                      Icon(isSelected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded, color: isSelected ? AppColors.brandGreen : AppColors.outlineLight, size: 20),
                    ],
                  ),
                ),
              ),
...
class _TravelPace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Travel Pace', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
        const SizedBox(height: 8),
        Row(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: const [BoxShadow(color: Color(0x0A004D40), offset: Offset(0, 4), blurRadius: 12)],
                  ),
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(p['icon'] as IconData, size: 24, color: isSelected ? AppColors.brandGreen : AppColors.outline),
                      const SizedBox(height: 6),
                      Text(label, style: GoogleFonts.inter(fontSize: 13, fontWeight: isSelected ? FontWeight.w700 : FontWeight.w600, color: isSelected ? AppColors.brandGreen : AppColors.brandCharcoal)),
                    ],
                  ),
                ),
              ),
...
class _Interests extends StatelessWidget {
  ...
  const _Interests({required this.selected, required this.onToggle, required this.maxInterests});
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('What excites you?', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
            Text('${selected.length}/$max', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: remaining == 0 ? AppColors.brandTerracotta : AppColors.brandGreen)),
          ],
        ),
        const SizedBox(height: 2),
        Text(
          remaining == 0 ? 'You\'ve reached the maximum ($max interests)' : 'SELECT UP TO $max — $remaining remaining',
          style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1.0, color: remaining == 0 ? AppColors.brandTerracotta : AppColors.outline),
        ),
          ...
          children: allKeys.map((label) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? AppColors.brandGreen : isDisabled ? AppColors.brandGrayLight : AppColors.white,
                  borderRadius: BorderRadius.circular(24),
                  border: Border.all(color: isSelected ? AppColors.brandGreen : AppColors.outlineLight.withOpacity(0.6), width: 1),
                  boxShadow: const [BoxShadow(color: Color(0x0A004D40), offset: Offset(0, 2), blurRadius: 10)],
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(icon, size: 16, color: isSelected ? Colors.white : isDisabled ? AppColors.outline : AppColors.brandCharcoal),
                    const SizedBox(width: 6),
                    Text(
                      label,
                      style: GoogleFonts.inter(fontSize: 13, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500, color: isSelected ? Colors.white : isDisabled ? AppColors.outline : AppColors.brandCharcoal),
                    ),
                  ],
                ),
              ),
...
class _Transportation extends StatelessWidget {
  final String? selected;
  final ValueChanged<String> onSelected;
  const _Transportation({required this.selected, required this.onSelected});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Transportation Mode', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.brandCharcoal)),
        const SizedBox(height: 8),
        Column(
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                        ...
                        child: Icon(icon, size: 22, color: isSelected ? AppColors.brandGreen : AppColors.outline),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(title, style: GoogleFonts.inter(fontSize: 14, fontWeight: isSelected ? FontWeight.w700 : FontWeight.w600, color: isSelected ? AppColors.brandGreen : AppColors.brandCharcoal)),
                            const SizedBox(height: 4),
                            Text(desc, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w400, height: 1.35, color: isSelected ? AppColors.brandGreen.withOpacity(0.85) : AppColors.outline)),
                          ],
                        ),
                      ),
                      const SizedBox(width: 8),
                      Icon(isSelected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded, color: isSelected ? AppColors.brandGreen : AppColors.outlineLight, size: 22),
                    ],
                  ),
                ),
              ),
      ...
      child: SafeArea(
        child: Container(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text('Craft My Itinerary', style: GoogleFonts.inter(fontWeight: FontWeight.w700, fontSize: 16)),
            style: ElevatedButton.styleFrom(
