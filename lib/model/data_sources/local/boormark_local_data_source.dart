...
class BookmarkLocalSource {
    ...
    return result.map((row) {
      ...
      final place = Place(
        ...
        placeTypes: row['place_types'] != null
            ? (row['place_types'] as String).split(',').map((e) => e.trim()).toList()
            : [],
  ...
  /// Insert or replace a bookmark locally.
  ...
  /// Delete a bookmark locally by ID.
  ...
  /// Cache a list of bookmarks with their associated places locally.
  Future<void> cacheBookmarksWithPlaces(List<BookmarkWithPlaceDTO> combined) async {
    final db = await _local.database;
